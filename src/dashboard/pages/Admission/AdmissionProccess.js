...
import { GetAllProccess } from "../../../slices/Admission/thunk";
...
const proccessTables = (props) => {
  const dispatch = useDispatch();
  const selectproccessState = (state) => state;

  useEffect(() => {
    dispatch(GetAllProccess());
  }, []);

  const proccesspageData = createSelector(selectproccessState, (state) => ({
  ...
  document.title = "Admission proccess";
      ...
      <Row>
        <Col xs={12}>
          <div className="page-content">
            <Container fluid>
              <BreadCrumb title="Admission proccess" pageTitle="Admission" />
              <Row>
                <Col lg={12}>
                  <Card>
                    <CardHeader>
                      <ToastContainer />
                      <h4 className="card-title mb-0">Admission proccess</h4>
                    </CardHeader>
                    <CardBody>
                      <div className="listjs-table" id="customerList">
                        <Row className="g-4 mb-3">
                          {/* <Col className="col-sm-auto">
                            <Link to="/dashboard/proccess/create">
                              <Button
                                color="success"
...
export default withRouter(proccessTables);
